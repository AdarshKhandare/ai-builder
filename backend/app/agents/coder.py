...
CODER_SYSTEM_PROMPT = (
    ...
    "OUTPUT FORMAT (STRICT — VIOLATIONS BREAK THE APP):\n"
    "- Do NOT wrap the output in markdown code fences. No ```html at the start, no ``` at the end.\n"
    "- The VERY FIRST characters must be <!DOCTYPE html>\n"
    "- The VERY LAST characters must be </html>\n"
    "- Output ONLY the HTML code — no explanations, no markdown, no comments "
