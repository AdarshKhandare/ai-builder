...
* ``GET /api/auth/login`` — start the OAuth flow. Generates a
  fresh ``state`` nonce, stores it in the short-lived
  ``forge_oauth_state`` cookie, and redirects the browser to
  GitHub's ``/login/oauth/authorize`` endpoint with the configured
  client id and ``read:user`` scope.
* ``GET /api/auth/callback`` — handle GitHub's redirect. First
  validates that the ``state`` query parameter matches the
  ``forge_oauth_state`` cookie (login-CSRF defence), then
  exchanges the ``code`` for an access token, fetches the user's
  profile from ``https://api.github.com/user``, upserts a
  :class:`User` row, issues a signed JWT, sets it as an
  ``httpOnly`` ``forge_token`` cookie, and redirects the browser
  to ``FRONTEND_URL + "/builder"``.
...
The ``forge_oauth_state`` cookie (set by ``/api/auth/login``)
carries the same flags plus a 10-minute ``Max-Age`` and a path
scoped to ``/api/auth/callback`` so it is auto-deleted on
success and cannot leak to other endpoints.
...
* State mismatch / missing state cookie — ``/api/auth/callback``
  returns ``403`` without ever contacting GitHub. This is the
  CSRF-defence path; a malicious cross-origin redirect would
  fail this check before leaking the user's GitHub identity.
...
# Short-lived CSRF state cookie. The value is a 24-byte URL-safe
# random nonce generated by :func:`login` and stored in a
# browser-only cookie so the :func:`callback` handler can confirm
# the redirect came from a login that THIS browser initiated.
# Without this check, a malicious site can embed an
# ``<img src="https://forge.example/api/auth/callback?code=ATTACKER_CODE">``
# tag and trick the victim's browser into logging in as the
# attacker. The 10-minute TTL is much longer than the GitHub
# round-trip in practice but short enough that a stale cookie
# cannot be reused to attack a later login.
_OAUTH_STATE_COOKIE = "forge_oauth_state"
...
async def login() -> Response:
    ...
    On success, the response is a 307 redirect to GitHub with a
    freshly generated ``state`` nonce attached to the URL. The same
    nonce is stored in a short-lived :data:`_OAUTH_STATE_COOKIE`
    cookie (``HttpOnly``, ``SameSite=Lax``, ``Secure`` in
    production) so :func:`callback` can verify the redirect
    originated from a login initiated by THIS browser. This is the
    standard CSRF defence for the OAuth ``code`` flow — without
    it, a third-party site can trick a victim's browser into
    completing an OAuth flow tied to the attacker's account
    (login-CSRF).

    Returns:
        A 307 redirect to ``https://github.com/login/oauth/authorize``
        with the configured client id, scope, and a fresh state
        nonce. A ``Set-Cookie`` header carrying the same state
        value is attached so the callback can verify it.
    ...
    try:
        ...
        # custom OAuth client. authlib generates a fresh ``state``
        # nonce if none is supplied; we keep it and mirror it
        # into a browser cookie so ``callback`` can match it
        # against the query parameter GitHub returns.
    ...
    # Mirror the state nonce into a short-lived browser cookie.
    # ``HttpOnly`` keeps it out of JS; ``SameSite=Lax`` allows it
    # on the top-level GET back from GitHub; ``Secure`` in
    # production so it is never sent over plain HTTP. The path
    # is scoped to the callback route so a logout on a different
    # page never accidentally carries the state value.
    response.set_cookie(
...
async def callback(
    ...
    0. **CSRF check** — confirm the ``state`` query parameter
       matches the ``forge_oauth_state`` cookie set by
       :func:`login`. This blocks login-CSRF: a malicious site
       cannot trick a victim's browser into completing an OAuth
       flow tied to the attacker's account because the attacker
       cannot read or set the victim's cookie. The cookie is
       deleted on successful (or failed) validation so the
       nonce is single-use.
    ...
    Args:
        request: The active FastAPI request (used to read the
            ``forge_oauth_state`` cookie set by :func:`login`).
        code: The OAuth ``code`` GitHub attached to the redirect.
        state: The OAuth ``state`` nonce echoed by GitHub; must
            equal the value of the ``forge_oauth_state`` cookie.
    ...
    Raises:
        HTTPException: ``403`` if the ``state`` query parameter
            is missing or does not match the ``forge_oauth_state``
            cookie (CSRF defence — checked BEFORE any GitHub
            contact); ``503`` if GitHub OAuth is not configured;
            ``502`` if the token exchange or user fetch fails;
            ``500`` for any other unexpected error.
    """
    # ------------------------------------------------------------------
    # CSRF defence — validate the state nonce BEFORE touching GitHub.
    # A missing cookie, a missing query parameter, or a mismatch are
    # all the same attack: an attacker has tricked a browser into
    # starting a callback it never initiated. We collapse them to a
    # single 403 so the response gives an attacker no information
    # about which check failed.
    # ------------------------------------------------------------------
    expected_state = request.cookies.get(_OAUTH_STATE_COOKIE)
    if not expected_state or expected_state != state:
        logger.warning(
            "OAuth state validation failed: cookie=%s query=%s",
            bool(expected_state),
            bool(state),
        )
        raise HTTPException(
    ...
    # Clear the state cookie — the nonce is single-use. We always
    # delete it on a fully-validated callback so a successful
    # login cannot be replayed (the next callback would have to
    # generate a fresh ``/api/auth/login`` request, which the
    # attacker cannot do because they cannot set the cookie).
